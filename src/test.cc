...
TEST_F(S21MatrixTest, CopyConstructor2) {
  ...
  EXPECT_EQ(0, matrix.GetRows());
  EXPECT_EQ(0, matrix.GetCols());
  EXPECT_EQ(0, matrix_copy.GetRows());
  EXPECT_EQ(0, matrix_copy.GetCols());
  EXPECT_EQ(1, matrix == matrix_copy);
}

TEST_F(S21MatrixTest, MoveConstructor1) {
  S21Matrix src_matrix(1, 1);
  EXPECT_EQ(1, src_matrix.GetRows());
  EXPECT_EQ(1, src_matrix.GetCols());
  S21Matrix dst_matrix(std::move(src_matrix));
  EXPECT_EQ(0, src_matrix.GetRows());
  EXPECT_EQ(0, src_matrix.GetCols());
  EXPECT_EQ(1, dst_matrix.GetRows());
  EXPECT_EQ(1, dst_matrix.GetCols());
}
...
TEST_F(S21MatrixTest, AdditionAssignmentOperator3) {
  EXPECT_THROW((*matrix_1x1) += (*matrix_2x3), std::invalid_argument);
}
...
TEST_F(S21MatrixTest, GetCols) {
  S21Matrix test;
  EXPECT_EQ(0, test.GetCols());
}

TEST_F(S21MatrixTest, GetCols1) { EXPECT_EQ(21, (*matrix_12x21).GetCols()); }
...
TEST_F(S21MatrixTest, SetColsException) {
  EXPECT_THROW((*matrix_2x3).SetCols(-9), std::invalid_argument);
}
...
TEST_F(S21MatrixTest, SumOperatorException) {
  EXPECT_THROW((*matrix_1x1) + (*matrix_21x21), std::invalid_argument);
}
...
TEST_F(S21MatrixTest, Transpose2) {
  S21Matrix transposed_matrix((*matrix_12x21).Transpose());
  EXPECT_EQ(21, transposed_matrix.GetRows());
  EXPECT_EQ(12, transposed_matrix.GetCols());
}
